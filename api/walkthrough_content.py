...
WALKTHROUGHS: list[Flow] = [
    ...
    Flow(
        ...
        roles=("owner",),
        status=PARTIAL,
        why="Marketing isn't a separate tool bolted on — it attaches to each stage of the same order pipeline. Capture, convert, recover, nurture.",
        steps=[
            "Capture (BUILT): every quote request becomes a lead with contact info and source.",
            "Attribution (NEXT): tag each lead/sale with where it came from (UTM/referrer) to see which channels convert.",
            "List sync (NEXT): push leads + buyers into your email tool automatically.",
            "Recover (NEXT): a started-but-unpaid checkout triggers a follow-up nudge.",
            "Nurture (NEXT): after delivery, an automated review request and referral ask.",
        ],
        structural_notes=[
